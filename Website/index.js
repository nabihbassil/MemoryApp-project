...
app.use('/assets', express.static(__dirname + '/assets')) //use for js files
...
app.post('/submit', urlEncodedBodyParser, (req, res) => {
  console.log('i got an addition request');

  //first we get the content inside the json file
  fs.readFile('./js/data.json', function readFileCallback(err, data) {
    if (err) {
      ...
      fs.writeFileSync('./js/data.json', JSON.stringify(obj, null, 2)); //write new object with the added data to the json file
...
app.post('/delete', urlEncodedBodyParser, (req, res) => {
  ...
  fs.readFile('./js/data.json', function readFileCallback(err, data) {
    if (err) {
      ...
      fs.writeFileSync('./js/data.json', JSON.stringify(obj, null, 2));
...
app.get('/exit', (req, res) => {
  console.log("exit");
  fs.writeFileSync('./js/data.json', '[]'); //delete all the data from the json file
